      ...
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
          <Link to="/" className="flex items-center gap-2" onClick={() => setOpen(false)}>
            <img src={logo} alt="Café Eternal" className="h-11 w-11 object-contain" />
            <span
            ...
            {links.map((l) => {
              ...
              return (
                <Link
                  key={l.to}
                  to={l.to}
                  className="font-display text-sm uppercase tracking-wider-2 transition-colors duration-200 relative"
                  style={{ color: isActive ? 'var(--olive)' : scrolled ? 'var(--brown)' : 'white' }}
                >
                  {l.label}
                  {isActive && (
                    <span className="absolute -bottom-1 left-0 right-0 h-px" style={{ background: 'var(--olive)' }} />
                  )}
                </Link>
              )
            })}
